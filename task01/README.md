...
- Root shell or sudo on a fresh target node/VM

---

## Validation result

Validated on a fresh Ubuntu 24.04 VM after running `sudo ./bootstrap.sh --run`:

```text
ubuntu-01:task01$ sudo ./validate.sh

=== Operating System ===
[PASS] Ubuntu 24.04

=== User Management ===
[PASS] Sysadmin account 'sysadmin' exists
[PASS] Sysadmin in sudo group
[PASS] Sudoers syntax valid

=== Kernel Prerequisites ===
[PASS] Swap disabled (none active)
[PASS] Module overlay loaded/built-in
[PASS] Module br_netfilter loaded/built-in
[PASS] sysctl net.ipv4.ip_forward = 1
[PASS] sysctl bridge-nf-call-iptables = 1
[PASS] sysctl bridge-nf-call-ip6tables = 1

=== Container Runtime ===
[PASS] containerd service active
[PASS] containerd SystemdCgroup = true
[PASS] Pause image = registry.k8s.io/pause:3.9

=== Kubernetes Components ===
[PASS] kubelet installed
[PASS] kubeadm installed
[PASS] kubelet enabled
[PASS] crictl endpoint configured
...
[PASS] Audit rule for user commands loaded
[PASS] TTY auditing enabled in PAM
[PASS] Audit log file present
...
Run this on a fresh Ubuntu 24.04 LTS node or VM. The playbook changes system
settings such as hostname, DNS, SSH, auditd, containerd, kubelet, and sysctl, so
it should not be tested on a personal workstation.
...
node running the playbook (mode `0600`, git-ignored). Users are forced to
...
(persisted to `.secrets/<user>.pass` on the node running Ansible). Re-runs reuse the
...
- TTY/session capture for interactive shell activity
